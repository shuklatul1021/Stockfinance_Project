...
def sell():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("You must provide a positive integer for shares", 400)

        shares = int(shares)

        stock_owned = next((stock for stock in stocks if stock["symbol"] == symbol), None)
        if not stock_owned:
            return apology("You do not own this stock", 400)

        if stock_owned["total_shares"] < shares:
